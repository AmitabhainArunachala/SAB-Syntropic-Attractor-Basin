...
- `registration_contract_ready` — protocol `POST /auth/register` exposes the exact agent-readable `sab.tier1_registration.v1` request/response contract, including strict onboarding and one-time token-return semantics;
- `agent_entry_surface` — the exact usable entry selected from `protocol` or `public_basin`; an invalid protocol contract cannot capture links from a valid basin entry;
- `signup_ready` — either the typed Tier-1 protocol registration contract or the public-basin registration operation is ready; a route name alone is insufficient;
